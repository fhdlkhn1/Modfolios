...
;(function( $, window, document, undefined ) {
				...
				if ( typeof (variations[num]) !== 'undefined' ) {
					...
					for ( var attr_name in variationAttributes ) {
						if ( variationAttributes.hasOwnProperty( attr_name ) ) {
							...
							var variation_out_of_stock = false; // Custom var by Minimog.
	...
	$.fn.wc_maybe_trigger_slide_position_reset = function( variation ) {
		...
		if ( reset_slide_position ) {
			...
			if ( $product_gallery.length > 0 ) {
				var swiperInstance  = $product_gallery.find( '.swiper' )[0].swiper,
				    $mainImageSlide = $product_gallery.find( '.swiper-slide.product-main-image' ),
				    index           = 0;
				if ( $mainImageSlide.length > 0 ) {
					index =
						typeof $mainImageSlide.data( 'swiper-slide-index' ) !== 'undefined' ? $mainImageSlide.data( 'swiper-slide-index' ) : $mainImageSlide.index();
				}
	...
	$.fn.wc_variations_image_update = function( variation ) {
		var $form             = this,
		    ...
		    $slider           = $product_gallery.find( '.minimog-main-swiper .swiper' ),
		...
		if ( variation && variation.variation_gallery_main_slides_html && $slider.length > 0 && $product_gallery.siblings( '.gallery-main-slides-o-html' ).length > 0 ) {
			...
			if ( $gallery_nav.length > 0 ) {
				var thumbSwiper = $gallery_nav.children( '.swiper-inner' ).children( '.swiper' )[0].swiper;
	...
	$.fn.wc_variations_slider_reset = function() {
		...
		if ( $product_gallery.hasClass( 'variation-gallery-changed' ) ) {
			...
			var swiper = $gallery_main.children( '.swiper-inner' ).children( '.swiper' )[0].swiper;
			swiper.removeAllSlides();
