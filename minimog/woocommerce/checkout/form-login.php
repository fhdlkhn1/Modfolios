...
<?php if ( $login_reminder_at_checkout ) : ?>
	<div class="checkout-login-link">
		<?php echo apply_filters( 'woocommerce_checkout_login_message', esc_html__( 'Returning customer?', 'minimog' ) ) . ' <a href="#" class="link-transition-02' . ( $popup_login_on ? ' open-modal-login' : ' showlogin' ) . '">' . esc_html__( 'Click here to login', 'minimog' ) . '</a>' ?>
	</div>
<?php endif; ?>
<?php
if ( ! $popup_login_on && ( $registration_at_checkout || $login_reminder_at_checkout ) ) :
