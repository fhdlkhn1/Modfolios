...
if ( ! class_exists( 'Minimog_Post' ) ) {
	class Minimog_Post {
		...
		public function frontend_scripts() {
			...
			if ( $this->is_archive() ) {
				$is_masonry_layout = Minimog::setting( 'blog_archive_masonry' );
		...
		function infinite_load() {
			...
			$query_vars = Minimog_Helper::data_clean( $_GET['query_vars'] );
			$settings   = isset( $_GET['settings'] ) ? Minimog_Helper::data_clean( $_GET['settings'] ) : array();
		...
		function meta_comment_count_template() {
			...
			<div class="post-comments">
				...
				printf( _n( '%s comment', '%s comments', $comment_count, 'minimog' ), $comment_count );
				?>
			</div>
			<?php
		}
