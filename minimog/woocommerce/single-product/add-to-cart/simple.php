...
$atc_base_class = 'single_add_to_cart_button ajax_add_to_cart button alt';

/**
 * Disabled theme add to cart ajax for WC Smart Coupon product.
 */
$coupon_titles = $product->get_meta( '_coupon_title' );
if( ! empty( $coupon_titles ) ) {
	$atc_base_class = str_replace('ajax_add_to_cart', '', $atc_base_class);
}

$add_to_cart_button_class = $atc_base_class;
		...
		<div class="entry-product-quantity-wrapper">
			...
			<?php if ( '1' === Minimog::setting( 'single_product_buy_now_enable' ) ) : ?>
				<button type="submit" name="minimog-buy-now" value="<?php echo esc_attr( $product->get_id() ); ?>"
				        class="<?php echo esc_attr( $atc_base_class ); ?> button-buy-now <?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ); ?>"
